...
const COPY = {
  "zh-CN": { title: "播放器设置", description: "配置播放方式、跳过、广告过滤与弹幕外观。", restricted: "当前账户没有修改播放器设置的权限；现有值仍可查看。",
    ...
    proxy: "代理播放模式", proxyHint: "控制视频播放时的网络请求策略", retry: "智能重试 (推荐)", retryHint: "直连优先，失败时尝试代理", none: "仅直连", noneHint: "不使用代理，失败则报错", always: "总是代理", alwaysHint: "所有请求都通过代理转发",
    safeBoundary: "安全边界：任何模式都不会让浏览器直接请求第三方媒体；实际播放保持 same-origin Worker 路径。",
  ...
  "zh-TW": { title: "播放器設定", description: "設定播放方式、略過、廣告過濾與彈幕外觀。", restricted: "目前帳戶沒有修改播放器設定的權限；仍可檢視現有值。",
    ...
    proxy: "代理播放模式", proxyHint: "相容偏好將由後續媒體策略使用；瀏覽器始終只請求同源 Worker。", retry: "智慧重試（建議）", retryHint: "直連上游失敗後嘗試代理策略", none: "僅直連", noneHint: "不做上游代理重試", always: "總是代理", alwaysHint: "所有上游媒體使用代理策略",
    safeBoundary: "安全邊界：任何模式都不會讓瀏覽器直接請求第三方媒體；實際播放維持 same-origin Worker 路徑。",
  ...
  en: { title: "Player settings", description: "Configure playback, skipping, ad filtering, and danmaku appearance.", restricted: "This account cannot change player settings; current values remain visible.",
    ...
    proxy: "Playback proxy mode", proxyHint: "This compatibility preference is consumed by later media strategy; the browser always calls the same-origin Worker.", retry: "Smart retry (recommended)", retryHint: "Try the proxy strategy after a direct upstream failure", none: "Direct only", noneHint: "Do not retry through the upstream proxy", always: "Always proxy", alwaysHint: "Use the proxy strategy for all upstream media",
    safeBoundary: "Safety boundary: no mode makes the browser call third-party media directly; playback remains on a same-origin Worker route.",
