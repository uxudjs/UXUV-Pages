...
const COPY = {
  "zh-CN": { title: "搜索结果排序", description: "选择搜索结果的默认排序方式", label: "默认排序方式", options: {
  ...
  "zh-TW": { title: "搜尋結果排序", description: "選擇搜尋結果的預設排序方式。", label: "預設排序方式", options: {
  ...
  en: { title: "Search result sorting", description: "Choose the default order for search results.", label: "Default sort order", options: {
...
} as const satisfies Record<string, { title: string; description: string; label: string; options: Record<SearchSortOption, string> }>;
...
export function SortSettings({ mode = "standard" }: Readonly<{ mode?: "standard" | "premium" }>) {
  ...
  return <SettingsSection id="sort" title={copy.title} description={copy.description}>
    <label className="settings-field-row sort-preference-row"><span>{copy.label}</span>
      <select aria-label={copy.title} value={preferences.sortBy} data-focusable onChange={(event) => preferences.setSortBy(event.target.value as SearchSortOption)}>
        {SEARCH_SORT_OPTIONS.map((option) => <option key={option} value={option}>{copy.options[option]}</option>)}
      </select>
    </label>
  </SettingsSection>;
}
