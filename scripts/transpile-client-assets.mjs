...
async function transpileFile(filePath) {
  ...
  if (UNSUPPORTED_LOGICAL_ASSIGNMENT.test(result.code)) {
    throw new Error(`${filePath} still contains logical assignment after ${TARGET} transpilation.`);
  }
...
async function main() {
  ...
  const assetRoots = roots.length > 0 ? roots : ["out/_next/static"];
  let existingRoots = 0;
  ...
  if (existingRoots === 0) {
    throw new Error(`No client asset directories found for ${TARGET} transpilation.`);
  }
  if (processedFiles === 0) {
    throw new Error(`No client JavaScript assets found for ${TARGET} transpilation.`);
  }
