...
const runtime = {
  ...
  capabilities: { premium: true, iptv: false, danmaku: false }, adKeywords: [], sources: { subscriptionSources: "", iptvSources: "", mergeSources: false, danmakuApiUrl: "" },
...
test.describe("KVideo T20 Premium settings", () => {
  ...
  test("fails closed on server authorization and isolates Premium source and player changes", async ({ page }) => {
    ...
    await display.getByRole("button", { name: /Group matching titles/ }).click();
