...
const outputPath = fileURLToPath(new URL("../kvideo-red-baseline.md", import.meta.url));
...
if (rows.length !== 273 || rows.some(({ status }) => status !== "RED")) {
  throw new Error(`Expected 273 RED capability rows, received ${rows.filter(({ status }) => status === "RED").length}.`);
}
